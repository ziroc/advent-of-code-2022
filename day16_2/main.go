...
func dfs2(cur int, rest []int, time int) (pressure int) {
	maxP := 0
	for i := 0; i < len(rest); i++ {
		to := rest[i]
		if to == -1 {
		...
		if costs[cur][to] < time {
			...
			copy(cc, rest)
			cc[i] = -1
			p := valves[to].releaseRate*(time-1-costs[cur][to]) + dfs2(to, cc, time-1-costs[cur][to])
			if p > maxP {
				maxP = p
			}
		}
	...
	fmt.Println("------")
	p2 := dfs(35, rest, 26)
	fmt.Println("^^^", p2, "cur is", cur)
	return u.Max(maxP, p2)
	// return maxP
}
